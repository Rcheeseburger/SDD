...
# This file contains the complete TransUNet implementation
...
def get_r50_l16_config():
    ...
    config.classifier = 'seg'
    config.pretrained_path = '../model/vit_checkpoint/imagenet21k/R50-ViT-L_16.npz'
    config.decoder_channels = (256, 128, 64, 16)
    config.skip_channels = [512, 256, 64, 16]
    config.n_classes = 2
    config.n_skip = 3
...
# ============= ResNet Skip Connection Implementation =============
...
class PreActBottleneck(nn.Module):
    ...
    def __init__(self, cin, cout=None, cmid=None, stride=1):
        ...
        self.conv2 = conv3x3(cmid, cmid, stride, bias=False)  # Original paper
...
class ResNetV2(nn.Module):
    ...
    def __init__(self, block_units, width_factor):
        ...
        self.body = nn.Sequential(
            *[nn.Sequential(
                *[PreActBottleneck(cin=width*4 if i > 0 else width,
                                   cout=width*4 if j == 0 else width*4,
                                   cmid=width) for j in range(block_units[i])]
...
# ============= Factory Function =============
def create_transunet(vit_name='R50-ViT-B_16', img_size=224, num_classes=2, pretrained_path=None):
    """
    Factory function to create TransUNet model
    
    Args:
        vit_name: ViT model variant ('ViT-B_16', 'ViT-L_16', 'R50-ViT-B_16', 'R50-ViT-L_16')
    ...
    # Update pretrained path if provided
    if pretrained_path:
        config.pretrained_path = pretrained_path
    
    # Set skip connections for ResNet variants
    if vit_name.find('R50') != -1:
        config.patches.grid = (int(img_size / config.patch_size), int(img_size / config.patch_size))
    
    model = TransUNet(config, img_size=img_size, num_classes=num_classes)
